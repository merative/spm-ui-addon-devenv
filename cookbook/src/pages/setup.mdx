...
6. Configure your React development environment variables. Before you start to work with React components, you must set node environment variables in `.env` and `.env.development` files.
Update the `.env` file, which sets the environment variables for production builds. Create a `.env.development` file, which supersedes the environment variables in `.env`, and set specific environment variables for development builds. For information about the enviroment variables to set in these files, see [Environment variables](environment-variables).
