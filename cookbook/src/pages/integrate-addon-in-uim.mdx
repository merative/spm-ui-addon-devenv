...
The `carbon-addon-devenv` package contains two sample add-on components. The following steps explain how to integrate a component with a UIM page with referece to the sample components.

However, you must complete some steps to test their integration with a UIM page.

You can view the components in Storybook by running the `npm` command to run Storybook.
